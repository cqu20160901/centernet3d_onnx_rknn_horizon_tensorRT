...
def detect(img_path, cal_path):
    origin_image = cv2.imread(img_path)
    origin_image = cv2.resize(origin_image, (input_w, input_h))
    image_h, image_w = origin_image.shape[:2]
    image = precess_image(origin_image, input_w, input_h)
    image = np.expand_dims(image, axis=0)

    ort_session = ort.InferenceSession('./Monodle.onnx')
